 ...
 */
 ...
 var shuffleDeck = function(deck) {
  // Your code here
    //i think the best idea is to grab a random value and then decrease the deck size
    //slice out the value mayhaps?
    //seems reasonable

    let lengthy = deck.length;

    Math.floor(Math.random() * lengthy)
    let empty = [];

    while(lengthy>0) {
        let rand = Math.floor(Math.random() * lengthy);
        let item = deck[rand];
        empty.push(item);
        deck.splice(rand, 1);
        lengthy--
